...
- ⏹️ **工作流控制**：支持在运行中停止绘图、文档、动画工作流
- 📊 **图片进度**：文档工作流中实时显示图片生成进度（当前/总数/描述）
- 🔍 **模型能力**：动态检测模型是否支持思考模式
...
- DeepSeek-Reasoner
- DeepSeek-R1 (Ollama)
- 其他支持 thinking 能力的模型（系统会自动检测）

其他模型不支持推理模式，即使启用也不会显示思考内容。

### 12. 工作流无法停止
如果点击停止按钮后工作流仍在运行，请检查：
- WebSocket 连接是否正常
- 后端服务是否响应 `/api/{workflow}/stop` 请求
